...
func NewSortCol(name string, asc bool) *SortCol {
	sc := &SortCol{Object: js.Global.Get("Object").New()}
	sc.Name = name
	sc.Asc = asc
	return sc
}

func SortedProjects(prjList []*Project, sortlist []*SortCol) []*Project {
	bylist := []lessFunc{}
	for _, s := range sortlist {
		bylist = append(bylist, by(s.Name, s.Asc))
	}

	ordered(bylist...).Sort(prjList)
