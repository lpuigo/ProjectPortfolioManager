...
func TestWeekCoverage(t *testing.T) {
	...
	for _, s := range []struct {
		...
		{"2018-01-01", "2018-05-30", 7, "much longer"},
		{monday, sunday, 7, "exactly the week"},
		{"2018-01-01", sunday, 7, "ends at end of week"},
		{monday, "2018-05-30", 7, "starts at begin of week"},
		{"2018-01-01", "2018-05-08", 2, "ends before week end"},
		{"2018-05-11", "2018-05-30", 3, "starts before week start"},
		{"2018-05-09", "2018-05-11", 3, "all within the week"},
		{"2018-05-08", "2018-05-08", 1, "one day project"},
		{monday, monday, 1, "one day Monday project"},
		{sunday, sunday, 1, "one day Sunday project"},
	} {
		res := calcWeekCoverage(monday, sunday, s.pBeg, s.pEnd)
		if res != s.expect {
			t.Errorf("case '%s': calcWeekCoverage(%s, %s, %s, %s) returns %f instead of %f", s.comment, monday, sunday, s.pBeg, s.pEnd, res, s.expect)
		}
	}
}
